function Table(options) {
    ...
    var table = {
        ...
        queryString: '',
        release: `acs${ACS_YEAR_NUMERIC}_5yr`
    }
        ...
        element.on('typeahead:selected', function(event, datum) {
            ...
            if (!datum['full_geoid'] && !!datum['sumlev']) {
                ...
                var url = table.buildDataURL(
                    'table', table.tableID, table.geoIDs, datum['full_geoid'], table.release
                );
                spinner.spin(spinnerTarget);
                window.location = url;
            }
    ...
    table.makeParentSelectWidget = function() {
        ...
        element.on('typeahead:selected', function(event, datum) {
            ...
            if (!!datum['full_geoid']) {
                ...
                var url = table.buildDataURL(
                    'table', table.tableID, table.geoIDs, table.primaryGeoID, table.release
                );
                spinner.spin(spinnerTarget);
                window.location = url;
            }
    ...
    table.buildDataURL = function(dataFormat, tableID, geoIDs, primaryGeoID, release) {
