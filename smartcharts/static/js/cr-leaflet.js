    ...
    GeoIDLayer: L.GeoJSON.extend({
        ...
        initialize: function(geoid_spec, options) {
            ...
            var options = L.extend({
                api_url: API_URL ? API_URL : 'https://sdcapi.datadrivendetroit.org',
                censusreporter_url: 'https://censusreporter.org',
                release: ACS_YEAR_NUMERIC ? `tiger${ACS_YEAR_NUMERIC}` : 'tiger2019',
                autoclick: true
            }, options);
...
CensusReporter.SummaryLevelLayer = CensusReporter.GeoJSONLayer.extend({
    ...
    initialize: function (summary_level, options, geojsonOptions) {
        ...
        var base_url = API_URL ? `${API_URL}/1.0/geo/`  : 'https://sdcapi.datadrivendetroit.org/1.0/geo/';
        var url = base_url + options.release + '/tiles/' + summary_level + '/{z}/{x}/{y}.geojson';
