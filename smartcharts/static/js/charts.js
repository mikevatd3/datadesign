...
function Chart(options) {
    ...
    chart.fillDataDrawerRows = function(selection, rowValues) {
        rowValues.forEach(function(d) {
            ...
            _.each(d.context.values, function(v, k) {
                // We want this to be one of three options for both primary value and error
                // 1. value if value is present
                // 2. Not enough data if data is missing or suppressed
                // 3. Nothing if it doesn't apply to the
                ...
                if (d.context.numerators[k] == null) {
                    rowData.push({ cellClass: 'value', cellContents: '' });
                } else {
